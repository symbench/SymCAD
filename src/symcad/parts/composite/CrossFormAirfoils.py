...
class CrossFormAirfoils(CompositeShape):
   ...
   ![CrossFormAirfoils](https://symbench.github.io/SymCAD/images/CrossFormAirfoils.png)
   ...
   def __create_cad__(params: Dict[str, float], _fully_displace: bool) -> Part.Solid:
      """Scripted CAD generation method for `CrossFormAirfoils`."""
      ...
      while x <= 1.009:
         points_upper.append(FreeCAD.Vector(x * chord_length_mm,
                                            CrossFormAirfoils.z_val(x, max_thickness_percent,
                                                                       chord_length_mm)))
      ...
      for i in range(4):
         ...
         body.Placement = FreeCAD.Placement(placement_vector,
                                            FreeCAD.Rotation(-curvature_tilt if i == 2 else
                                                             (curvature_tilt if i == 0 else 0),
                                                             -curvature_tilt if i == 1 else
                                                             (curvature_tilt if i == 3 else 0),
                                                             i * 90.0))
         bodies.append(body)
   ...
   def set_geometry(self, *, max_thickness_percent: Union[float, None],
                             ...
                             curvature_tilt_deg: Union[float, None]) -> CrossFormAirfoils:
   ...
   def displaced_volume(self) -> Union[float, Expr]:
      ...
      while x <= 1.009:
         points_upper.append((x * self.geometry.chord_length,
                              CrossFormAirfoils.z_val(x, self.geometry.max_thickness,
      ...
      return 4.0 * h * area * self.geometry.span
