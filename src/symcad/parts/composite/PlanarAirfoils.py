...
class PlanarAirfoils(CompositeShape):
   ...
   ![PlanarAirfoils](https://symbench.github.io/SymCAD/images/PlanarAirfoils.png)
   ...
   def __create_cad__(params: Dict[str, float], _fully_displace: bool) -> Part.Solid:
      """Scripted CAD generation method for `PlanarAirfoils`."""
      ...
      while x <= 1.009:
         points_upper.append(FreeCAD.Vector(x * chord_length_mm,
                                            PlanarAirfoils.z_val(x, max_thickness_percent,
                                                                    chord_length_mm)))
      ...
      for i in range(2):
         ...
         pad.Profile = sketch
         placement_vector = \
            FreeCAD.Vector(0, -separation_radius_mm if i == 0 else separation_radius_mm, 0)
         body.Placement = \
            FreeCAD.Placement(placement_vector,
                              FreeCAD.Rotation(-curvature_tilt if i == 1 else curvature_tilt,
                                                0, i * 180.0))
         bodies.append(body)
   ...
   def set_geometry(self, *, max_thickness_percent: Union[float, None],
                             ...
                             curvature_tilt_deg: Union[float, None]) -> PlanarAirfoils:
   ...
   def displaced_volume(self) -> Union[float, Expr]:
      ...
      while x <= 1.009:
         points_upper.append((x * self.geometry.chord_length,
                              PlanarAirfoils.z_val(x, self.geometry.max_thickness,
      ...
      return 2.0 * h * area * self.geometry.span
   ...
   def unoriented_width(self) -> Union[float, Expr]:
      cosine = cos(self.geometry.curvature_tilt * math.pi / 180.0)
      return (2.0 * cosine * self.geometry.span) + (2.0 * self.geometry.separation_radius)
