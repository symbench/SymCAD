...
class HemisphericalCapsule(CompositeShape):
   ...
   ![HemisphericalCapsule](https://symbench.github.io/SymCAD/images/HemisphericalCapsule.png)
   ...
   def __create_cad__(params: Dict[str, float], fully_displace: bool) -> Part.Solid:
      """Scripted CAD generation method for `HemisphericalCapsule`."""
      ...
      inner_endcap_radius_mm = 1000.0 * (params['cylinder_radius'] - params['endcap_thickness'])
      front = Part.makeSphere(outer_cylinder_radius_mm,
                              FreeCAD.Vector(0, 0, 0),
                              FreeCAD.Vector(-1, 0, 0), 0, 90, 360)
      rear = Part.makeSphere(outer_cylinder_radius_mm,
                             FreeCAD.Vector(0, 0, 0),
                             FreeCAD.Vector(1, 0, 0), 0, 90, 360)
      if fully_displace:
         ...
         inner_front = Part.makeSphere(inner_endcap_radius_mm,
                                       FreeCAD.Vector(0, 0, 0),
                                       FreeCAD.Vector(-1, 0, 0), 0, 90, 360)
         inner_rear = Part.makeSphere(inner_endcap_radius_mm,
                                      FreeCAD.Vector(0, 0, 0),
                                      FreeCAD.Vector(1, 0, 0), 0, 90, 360)
         front = front.cut(inner_front)
         rear = rear.cut(inner_rear)
   ...
   def set_geometry(self, *, cylinder_radius_m: Union[float, None],
                             ...
                             endcap_thickness_m: Union[float, None]) -> HemisphericalCapsule:
   ...
   def unoriented_length(self) -> Union[float, Expr]:
      return (2.0 * self.geometry.cylinder_radius) + self.geometry.cylinder_length
