...
import os
from symcad.parts import SymPart, NortekDVL1000_4000mDvl
...
def test_construction(print_output: bool = False):
   ...
   shape = NortekDVL1000_4000mDvl(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(NortekDVL1000_4000mDvl, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, NortekDVL1000_4000mDvl)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 2.75) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 1.227) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = NortekDVL1000_4000mDvl(shape_identifier)
   shape_rolled = NortekDVL1000_4000mDvl(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = NortekDVL1000_4000mDvl(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = NortekDVL1000_4000mDvl(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = NortekDVL1000_4000mDvl(shape_identifier)\
