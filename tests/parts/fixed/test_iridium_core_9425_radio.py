...
import os
from symcad.parts import SymPart, IridiumCore9523Radio
...
def test_construction(print_output: bool = False):
   ...
   shape = IridiumCore9523Radio(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(IridiumCore9523Radio, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, IridiumCore9523Radio)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 0.032) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = IridiumCore9523Radio(shape_identifier)
   shape_rolled = IridiumCore9523Radio(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = IridiumCore9523Radio(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = IridiumCore9523Radio(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = IridiumCore9523Radio(shape_identifier)\
