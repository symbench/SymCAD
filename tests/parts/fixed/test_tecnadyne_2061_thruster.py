...
import os
from symcad.parts import SymPart, TecnadyneModel2061Thruster
...
def test_construction(print_output: bool = False):
   ...
   shape = TecnadyneModel2061Thruster(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(TecnadyneModel2061Thruster, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, TecnadyneModel2061Thruster)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 21.0) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 14.525) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = TecnadyneModel2061Thruster(shape_identifier)
   shape_rolled = TecnadyneModel2061Thruster(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = TecnadyneModel2061Thruster(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = TecnadyneModel2061Thruster(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = TecnadyneModel2061Thruster(shape_identifier)\
