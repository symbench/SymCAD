...
import os
from symcad.parts import SymPart, TeledyneBenthosATM926AcousticModem
...
def test_construction(print_output: bool = False):
   ...
   shape = TeledyneBenthosATM926AcousticModem(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(TeledyneBenthosATM926AcousticModem, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, TeledyneBenthosATM926AcousticModem)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 4.1) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 1.853) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = TeledyneBenthosATM926AcousticModem(shape_identifier)
   shape_rolled = TeledyneBenthosATM926AcousticModem(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = TeledyneBenthosATM926AcousticModem(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = TeledyneBenthosATM926AcousticModem(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = TeledyneBenthosATM926AcousticModem(shape_identifier)\
