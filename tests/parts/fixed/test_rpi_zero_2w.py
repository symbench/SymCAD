...
import os
from symcad.parts import SymPart, RaspberryPiZero2Computer
...
def test_construction(print_output: bool = False):
   ...
   shape = RaspberryPiZero2Computer(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(RaspberryPiZero2Computer, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, RaspberryPiZero2Computer)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 0.016) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = RaspberryPiZero2Computer(shape_identifier)
   shape_rolled = RaspberryPiZero2Computer(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = RaspberryPiZero2Computer(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = RaspberryPiZero2Computer(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = RaspberryPiZero2Computer(shape_identifier)\
