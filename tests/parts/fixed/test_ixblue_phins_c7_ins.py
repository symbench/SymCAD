...
import os
from symcad.parts import SymPart, iXbluePhinsCompactC7Ins
...
def test_construction(print_output: bool = False):
   ...
   shape = iXbluePhinsCompactC7Ins(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(iXbluePhinsCompactC7Ins, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, iXbluePhinsCompactC7Ins)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 3.5) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = iXbluePhinsCompactC7Ins(shape_identifier)
   shape_rolled = iXbluePhinsCompactC7Ins(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = iXbluePhinsCompactC7Ins(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = iXbluePhinsCompactC7Ins(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = iXbluePhinsCompactC7Ins(shape_identifier)\
