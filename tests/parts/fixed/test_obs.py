...
import os
from symcad.parts import SymPart, OceanBottomSeismometer
...
def test_construction(print_output: bool = False):
   ...
   shape = OceanBottomSeismometer(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(OceanBottomSeismometer, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, OceanBottomSeismometer)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 91.232) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 46.9) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = OceanBottomSeismometer(shape_identifier)
   shape_rolled = OceanBottomSeismometer(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = OceanBottomSeismometer(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = OceanBottomSeismometer(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = OceanBottomSeismometer(shape_identifier)\
