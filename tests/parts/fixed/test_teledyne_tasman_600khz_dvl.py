...
import os
from symcad.parts import SymPart, TeledyneTasman600kHzDvl
...
def test_construction(print_output: bool = False):
   ...
   shape = TeledyneTasman600kHzDvl(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(TeledyneTasman600kHzDvl, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, TeledyneTasman600kHzDvl)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(shape.mass - 8.7) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 4.4) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = TeledyneTasman600kHzDvl(shape_identifier)
   shape_rolled = TeledyneTasman600kHzDvl(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = TeledyneTasman600kHzDvl(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = TeledyneTasman600kHzDvl(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = TeledyneTasman600kHzDvl(shape_identifier)\
