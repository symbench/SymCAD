...
import os
from symcad.parts import SymPart, TridentSensorsDualGpsIridiumAntenna
...
def test_construction(print_output: bool = False):
   ...
   shape = TridentSensorsDualGpsIridiumAntenna(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(TridentSensorsDualGpsIridiumAntenna, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, TridentSensorsDualGpsIridiumAntenna)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(cad_props['ylen'] - shape.unoriented_width) < 0.001
   assert abs(cad_props['zlen'] - shape.unoriented_height) < 0.001
   assert abs(cad_props['cg_x'] - shape.unoriented_center_of_gravity[0]) < 0.001
   assert abs(cad_props['cg_y'] - shape.unoriented_center_of_gravity[1]) < 0.001
   assert abs(cad_props['cg_z'] - shape.unoriented_center_of_gravity[2]) < 0.001
   assert abs(cad_props['cb_x'] - shape.unoriented_center_of_buoyancy[0]) < 0.001
   assert abs(cad_props['cb_y'] - shape.unoriented_center_of_buoyancy[1]) < 0.001
   assert abs(cad_props['cb_z'] - shape.unoriented_center_of_buoyancy[2]) < 0.001
   ...
   assert abs(shape.mass - 0.13) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 0.055) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = TridentSensorsDualGpsIridiumAntenna(shape_identifier)
   shape_rolled = TridentSensorsDualGpsIridiumAntenna(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = TridentSensorsDualGpsIridiumAntenna(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = TridentSensorsDualGpsIridiumAntenna(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = TridentSensorsDualGpsIridiumAntenna(shape_identifier)\
