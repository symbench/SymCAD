...
import os
from symcad.parts import SymPart, TecnadyneModel2051Thruster
...
def test_construction(print_output: bool = False):
   ...
   shape = TecnadyneModel2051Thruster(shape_identifier)

   # Assert that the shape type is as expected
   assert issubclass(TecnadyneModel2051Thruster, SymPart)
   assert isinstance(shape, SymPart)
   assert isinstance(shape, TecnadyneModel2051Thruster)
...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(0.127 - shape.unoriented_width) < 0.001
   assert abs(0.127 - shape.unoriented_height) < 0.001
   assert abs(cad_props['cg_x'] - shape.unoriented_center_of_gravity[0]) < 0.001
   assert abs(0.5 * 0.127 - shape.unoriented_center_of_gravity[1]) < 0.001
   assert abs(0.5 * 0.127 - shape.unoriented_center_of_gravity[2]) < 0.001
   assert abs(cad_props['cb_x'] - shape.unoriented_center_of_buoyancy[0]) < 0.001
   assert abs(0.5 * 0.127 - shape.unoriented_center_of_buoyancy[1]) < 0.001
   assert abs(0.5 * 0.127 - shape.unoriented_center_of_buoyancy[2]) < 0.001
   ...
   assert abs(shape.mass - 18.0) < 0.001
   assert abs(shape.mass - (shape.displaced_volume * 1027.0) - 15.273) < 0.001
...
def test_cad(_print_output: bool = False):
   ...
   shape = TecnadyneModel2051Thruster(shape_identifier)
   shape_rolled = TecnadyneModel2051Thruster(shape_identifier)\
      .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_pitched = TecnadyneModel2051Thruster(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_yawed = TecnadyneModel2051Thruster(shape_identifier)\
      .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_rotated = TecnadyneModel2051Thruster(shape_identifier)\
