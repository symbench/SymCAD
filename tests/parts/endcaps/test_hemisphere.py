...
from symcad.parts import Hemisphere, SymPart

symbolic_identifier = 'hemisphere_symbolic'
hybrid_identifier = 'hemisphere_hybrid'
concrete_identifier = 'hemisphere_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = Hemisphere(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(Hemisphere, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, Hemisphere)
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = Hemisphere(symbolic_identifier)
   shape_hybrid = Hemisphere(hybrid_identifier).set_geometry(radius_m=None, thickness_m=0.01)
   shape_concrete = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)\
   ...
   shape_hybrid.set_geometry(radius_m=None, thickness_m=0.01)
   shape_concrete.set_geometry(radius_m=1.1, thickness_m=0.01)
   ...
   assert shape_concrete.geometry.radius == 1.1
...
def test_built_ins(print_output: bool = False):
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.radius == 4.4
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.radius == 1.1 / 4.0
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = Hemisphere(symbolic_identifier)
   shape_hybrid = Hemisphere(hybrid_identifier).set_geometry(radius_m=None, thickness_m=0.01)
   shape_concrete = Hemisphere(concrete_identifier, 2.0).set_geometry(radius_m=1.1, thickness_m=0.01)
   ...
   assert abs(cad_props['cg_x'] - shape_concrete.unoriented_center_of_gravity[0]) < 0.01
   assert abs(cad_props['cg_y'] - shape_concrete.unoriented_center_of_gravity[1]) < 0.01
   assert abs(cad_props['cg_z'] - shape_concrete.unoriented_center_of_gravity[2]) < 0.01
   assert abs(cad_props['cb_x'] - shape_concrete.unoriented_center_of_buoyancy[0]) < 0.01
   assert abs(cad_props['cb_y'] - shape_concrete.unoriented_center_of_buoyancy[1]) < 0.01
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = Hemisphere(concrete_identifier, 2.0)\
      .set_geometry(radius_m=1.1, thickness_m=0.01)\
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)
   shape_concrete_rolled = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)\
                                                          .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)\
                                                           .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)\
                                                         .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = Hemisphere(concrete_identifier).set_geometry(radius_m=1.1, thickness_m=0.01)\
                                                           .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('hemisphere.FCStd', 'freecad')
   shape_concrete_rolled.export('hemisphere_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('hemisphere_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('hemisphere_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('hemisphere_rotated.FCStd', 'freecad')
   os.remove('hemisphere.FCStd')
   os.remove('hemisphere_rolled.FCStd')
   os.remove('hemisphere_pitched.FCStd')
   os.remove('hemisphere_yawed.FCStd')
   os.remove('hemisphere_rotated.FCStd')
