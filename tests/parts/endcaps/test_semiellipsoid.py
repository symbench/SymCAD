...
from symcad.parts import Semiellipsoid, SymPart

symbolic_identifier = 'semiellipsoid_symbolic'
hybrid_identifier = 'semiellipsoid_hybrid'
concrete_identifier = 'semiellipsoid_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = Semiellipsoid(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(Semiellipsoid, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, Semiellipsoid)
   ...
   assert shape_symbolic.geometry.minor_radius == 0.5 * sympy.Symbol(symbolic_identifier + '_major_radius')
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = Semiellipsoid(symbolic_identifier)
   shape_hybrid = Semiellipsoid(hybrid_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=None, thickness_m=0.01, minor_depends_on_major=False)
   shape_concrete = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
   ...
   assert shape_hybrid.geometry.major_radius == 4.0
   ...
   assert shape_concrete.geometry.major_radius == 4.0
...
def test_built_ins(print_output: bool = False):
   ...
   assert shape_symbolic.geometry.minor_radius == 2.0 * sympy.Symbol(symbolic_identifier + '_major_radius')
   ...
   # Assert that all hybrid shape parameters are as expected
   assert shape_hybrid.geometry.major_radius == 16.0
   assert shape_hybrid.geometry.minor_radius == 8.0
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.major_radius == 16.0
   assert shape_concrete.geometry.minor_radius == 8.0
   ...
   assert shape_symbolic.geometry.minor_radius == 0.5 * sympy.Symbol(symbolic_identifier + '_major_radius') / 4.0
   ...
   # Assert that all hybrid shape parameters are as expected
   assert shape_hybrid.geometry.major_radius == 1.0
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.major_radius == 1.0
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = Semiellipsoid(symbolic_identifier)
   shape_hybrid = Semiellipsoid(hybrid_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=None, thickness_m=0.01, minor_depends_on_major=False)
   shape_concrete = Semiellipsoid(concrete_identifier, 2.0).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)
   ...
   assert abs(cad_props['cg_x'] - shape_concrete.unoriented_center_of_gravity[0]) < 0.1
   assert abs(cad_props['cg_y'] - shape_concrete.unoriented_center_of_gravity[1]) < 0.1
   assert abs(cad_props['cg_z'] - shape_concrete.unoriented_center_of_gravity[2]) < 0.1
   assert abs(cad_props['cb_x'] - shape_concrete.unoriented_center_of_buoyancy[0]) < 0.1
   assert abs(cad_props['cb_y'] - shape_concrete.unoriented_center_of_buoyancy[1]) < 0.1
   assert abs(cad_props['cb_z'] - shape_concrete.unoriented_center_of_buoyancy[2]) < 0.1
   assert abs(cad_props['min_x']) < 0.001
   assert abs(cad_props['min_y']) < 0.001
   assert abs(cad_props['min_z']) < 0.001
   assert abs(cad_props['mass'] - shape_concrete.mass) < 0.05
   assert abs(cad_props['material_volume'] - shape_concrete.material_volume) < 0.05
   assert abs(cad_props['displaced_volume'] - shape_concrete.displaced_volume) < 1.0
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = Semiellipsoid(concrete_identifier, 2.0)\
      .set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
   ...
   print(props, shape_concrete.oriented_center_of_gravity)
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)
   shape_concrete_rolled = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
                                                             .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
                                                              .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
                                                            .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = Semiellipsoid(concrete_identifier).set_geometry(major_axis_radius_m=4.0, minor_axis_radius_m=2.0, thickness_m=0.01)\
                                                              .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('semiellipsoid.FCStd', 'freecad')
   shape_concrete_rolled.export('semiellipsoid_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('semiellipsoid_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('semiellipsoid_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('semiellipsoid_rotated.FCStd', 'freecad')
   os.remove('semiellipsoid.FCStd')
   os.remove('semiellipsoid_rolled.FCStd')
   os.remove('semiellipsoid_pitched.FCStd')
   os.remove('semiellipsoid_yawed.FCStd')
   os.remove('semiellipsoid_rotated.FCStd')
