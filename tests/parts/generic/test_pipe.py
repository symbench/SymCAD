...
from symcad.parts import Pipe, SymPart

symbolic_identifier = 'pipe_symbolic'
hybrid_identifier = 'pipe_hybrid'
concrete_identifier = 'pipe_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = Pipe(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(Pipe, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, Pipe)
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = Pipe(symbolic_identifier)
   shape_hybrid = Pipe(hybrid_identifier).set_geometry(radius_m=1.0, height_m=None, thickness_m=0.01)
   shape_concrete = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
   ...
   shape_symbolic.set_geometry(radius_m=None, height_m=None, thickness_m=None)
   shape_hybrid.set_geometry(radius_m=1.0, height_m=None, thickness_m=0.01)
   shape_concrete.set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)
   ...
   assert shape_hybrid.geometry.radius == 1.0
   ...
   assert shape_concrete.geometry.radius == 1.0
...
def test_built_ins(print_output: bool = False):
   ...
   # Assert that all hybrid shape parameters are as expected
   assert shape_hybrid.geometry.radius == 4.0
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.radius == 4.0
   ...
   # Assert that all hybrid shape parameters are as expected
   assert shape_hybrid.geometry.radius == 1.0 / 4.0
   ...
   # Assert that all concrete shape parameters are as expected
   assert shape_concrete.geometry.radius == 1.0 / 4.0
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = Pipe(symbolic_identifier)
   shape_hybrid = Pipe(hybrid_identifier).set_geometry(radius_m=1.0, height_m=None, thickness_m=0.01)
   shape_concrete = Pipe(concrete_identifier, 2.0).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = Pipe(concrete_identifier, 2.0)\
      .set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
   ...
   assert abs(shape_concrete.oriented_width - props['ylen']) < 0.02
   assert abs(shape_concrete.oriented_height - props['zlen']) < 0.01
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)
   shape_concrete_rolled = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
                                                    .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
                                                     .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
                                                   .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = Pipe(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0, thickness_m=0.01)\
                                                     .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('pipe.FCStd', 'freecad')
   shape_concrete_rolled.export('pipe_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('pipe_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('pipe_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('pipe_rotated.FCStd', 'freecad')
   os.remove('pipe.FCStd')
   os.remove('pipe_rolled.FCStd')
   os.remove('pipe_pitched.FCStd')
   os.remove('pipe_yawed.FCStd')
   os.remove('pipe_rotated.FCStd')
