...
def test_geometric_properties(print_output: bool = False):
   ...
   assert abs(cad_props['material_volume'] - shape_concrete.material_volume) < 0.02
   assert abs(cad_props['displaced_volume'] - shape_concrete.displaced_volume) < 0.02
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = EllipsoidalCap(concrete_identifier, 2.0)\
      .set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=0.3)\
