...
from symcad.parts import Cylinder, SymPart

symbolic_identifier = 'cylinder_symbolic'
hybrid_identifier = 'cylinder_hybrid'
concrete_identifier = 'cylinder_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = Cylinder(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(Cylinder, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, Cylinder)
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = Cylinder(symbolic_identifier)
   shape_hybrid = Cylinder(hybrid_identifier).set_geometry(radius_m=1.0, height_m=None)
   shape_concrete = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)\
   ...
   shape_symbolic.set_geometry(radius_m=None, height_m=None)
   shape_hybrid.set_geometry(radius_m=1.0, height_m=None)
   shape_concrete.set_geometry(radius_m=1.0, height_m=3.0)
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = Cylinder(symbolic_identifier)
   shape_hybrid = Cylinder(hybrid_identifier).set_geometry(radius_m=1.0, height_m=None)
   shape_concrete = Cylinder(concrete_identifier, 2.0).set_geometry(radius_m=1.0, height_m=3.0)
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = Cylinder(concrete_identifier, 2.0)\
      .set_geometry(radius_m=1.0, height_m=3.0)\
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)
   shape_concrete_rolled = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)\
                                                        .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)\
                                                         .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)\
                                                       .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = Cylinder(concrete_identifier).set_geometry(radius_m=1.0, height_m=3.0)\
                                                         .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('cylinder.FCStd', 'freecad')
   shape_concrete_rolled.export('cylinder_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('cylinder_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('cylinder_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('cylinder_rotated.FCStd', 'freecad')
   os.remove('cylinder.FCStd')
   os.remove('cylinder_rolled.FCStd')
   os.remove('cylinder_pitched.FCStd')
   os.remove('cylinder_yawed.FCStd')
   os.remove('cylinder_rotated.FCStd')
