...
from symcad.parts import Prism, SymPart

symbolic_identifier = 'prism_symbolic'
hybrid_identifier = 'prism_hybrid'
concrete_identifier = 'prism_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = Prism(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(Prism, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, Prism)
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = Prism(symbolic_identifier)
   shape_hybrid = Prism(hybrid_identifier).set_geometry(num_edges=6, edge_length_m=None, height_m=3.0)
   shape_concrete = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = Prism(symbolic_identifier)
   shape_hybrid = Prism(hybrid_identifier).set_geometry(num_edges=6, edge_length_m=None, height_m=3.0)
   shape_concrete = Prism(concrete_identifier, 2.0).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = Prism(concrete_identifier, 2.0)\
      .set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)
   shape_concrete_rolled = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
                                                     .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
                                                      .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
                                                    .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = Prism(concrete_identifier).set_geometry(num_edges=6, edge_length_m=1.0, height_m=3.0)\
                                                      .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('prism.FCStd', 'freecad')
   shape_concrete_rolled.export('prism_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('prism_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('prism_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('prism_rotated.FCStd', 'freecad')
   os.remove('prism.FCStd')
   os.remove('prism_rolled.FCStd')
   os.remove('prism_pitched.FCStd')
   os.remove('prism_yawed.FCStd')
   os.remove('prism_rotated.FCStd')
