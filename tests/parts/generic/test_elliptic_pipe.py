...
from symcad.parts import EllipticPipe, SymPart

symbolic_identifier = 'elliptic_pipe_symbolic'
hybrid_identifier = 'elliptic_pipe_hybrid'
concrete_identifier = 'elliptic_pipe_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = EllipticPipe(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(EllipticPipe, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, EllipticPipe)
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = EllipticPipe(symbolic_identifier)
   shape_hybrid = EllipticPipe(hybrid_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=None, height_m=None, thickness_m=0.01)
   shape_concrete = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
   ...
   shape_symbolic.set_geometry(major_radius_m=None, minor_radius_m=None, height_m=None, thickness_m=None)
   shape_hybrid.set_geometry(major_radius_m=1.5, minor_radius_m=None, height_m=None, thickness_m=0.01)
   shape_concrete.set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)
   ...
   assert shape_hybrid.geometry.minor_radius == sympy.Symbol(hybrid_identifier + '_minor_radius')
...
def test_built_ins(print_output: bool = False):
   ...
   assert shape_hybrid.geometry.minor_radius == 4.0 * sympy.Symbol(hybrid_identifier + '_minor_radius')
   ...
   assert shape_hybrid.geometry.minor_radius == sympy.Symbol(hybrid_identifier + '_minor_radius') / 4.0
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = EllipticPipe(symbolic_identifier)
   shape_hybrid = EllipticPipe(hybrid_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=None, height_m=None, thickness_m=0.01)
   shape_concrete = EllipticPipe(concrete_identifier, 2.0).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)
   ...
   assert abs(cad_props['surface_area'] - shape_concrete.surface_area) < 0.01
...
def test_oriented_properties(print_output: bool = False):
   ...
   shape_concrete = EllipticPipe(concrete_identifier, 2.0)\
      .set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
   ...
   assert abs(shape_concrete.oriented_width - props['ylen']) < 0.01
   assert abs(shape_concrete.oriented_height - props['zlen']) < 0.02
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)
   shape_concrete_rolled = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
                                                            .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
                                                             .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
                                                           .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = EllipticPipe(concrete_identifier).set_geometry(major_radius_m=1.5, minor_radius_m=0.5, height_m=3.0, thickness_m=0.01)\
                                                             .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('elliptic_pipe.FCStd', 'freecad')
   shape_concrete_rolled.export('elliptic_pipe_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('elliptic_pipe_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('elliptic_pipe_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('elliptic_pipe_rotated.FCStd', 'freecad')
   os.remove('elliptic_pipe.FCStd')
   os.remove('elliptic_pipe_rolled.FCStd')
   os.remove('elliptic_pipe_pitched.FCStd')
   os.remove('elliptic_pipe_yawed.FCStd')
   os.remove('elliptic_pipe_rotated.FCStd')
