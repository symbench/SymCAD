...
from symcad.parts import SymmetricAirfoil, SymPart

symbolic_identifier = 'airfoil_symbolic'
hybrid_identifier = 'airfoil_hybrid'
concrete_identifier = 'airfoil_concrete'

def test_construction(print_output: bool = False):

   # Construct a default instance of the shape
   shape_symbolic = SymmetricAirfoil(symbolic_identifier)

   # Assert that the shape type is as expected
   assert issubclass(SymmetricAirfoil, SymPart)
   assert isinstance(shape_symbolic, SymPart)
   assert isinstance(shape_symbolic, SymmetricAirfoil)
   ...
   assert shape_symbolic.geometry.material_thickness == sympy.Symbol(symbolic_identifier + '_material_thickness')
...
def test_setting(print_output: bool = False):
   ...
   shape_symbolic = SymmetricAirfoil(symbolic_identifier)
   shape_hybrid = SymmetricAirfoil(hybrid_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=None, material_thickness_m=0.01)
   shape_concrete = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)\
   ...
   assert shape_hybrid.geometry.material_thickness == 0.01
   ...
   assert shape_concrete.geometry.material_thickness == 0.01
...
def test_geometric_properties(print_output: bool = False):
   ...
   shape_symbolic = SymmetricAirfoil(symbolic_identifier)
   shape_hybrid = SymmetricAirfoil(hybrid_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=None, material_thickness_m=0.01)
   shape_concrete = SymmetricAirfoil(concrete_identifier, 2.0).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)
...
def test_cad(_print_output: bool = False):
   ...
   shape_concrete = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)
   shape_concrete_rolled = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)\
                                                                .set_orientation(roll_deg=20.0, pitch_deg=0.0, yaw_deg=0.0)
   shape_concrete_pitched = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)\
                                                                 .set_orientation(roll_deg=0.0, pitch_deg=20.0, yaw_deg=0.0)
   shape_concrete_yawed = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)\
                                                               .set_orientation(roll_deg=0.0, pitch_deg=0.0, yaw_deg=20.0)
   shape_concrete_rotated = SymmetricAirfoil(concrete_identifier).set_geometry(max_thickness_percent=0.12, chord_length_m=0.16, span_m=0.2, material_thickness_m=0.01)\
                                                                 .set_orientation(roll_deg=-20.0, pitch_deg=-20.0, yaw_deg=-20.0)

   # Export FreeCAD versions of the rotated shapes
   shape_concrete.export('airfoil.FCStd', 'freecad')
   shape_concrete_rolled.export('airfoil_rolled.FCStd', 'freecad')
   shape_concrete_pitched.export('airfoil_pitched.FCStd', 'freecad')
   shape_concrete_yawed.export('airfoil_yawed.FCStd', 'freecad')
   shape_concrete_rotated.export('airfoil_rotated.FCStd', 'freecad')
   os.remove('airfoil.FCStd')
   os.remove('airfoil_rolled.FCStd')
   os.remove('airfoil_pitched.FCStd')
   os.remove('airfoil_yawed.FCStd')
   os.remove('airfoil_rotated.FCStd')
